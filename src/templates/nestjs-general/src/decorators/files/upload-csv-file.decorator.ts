...
import { validateCsvContent } from './csv-validator/csv-validator';
import {
  LoggerHelperService,
  TrackerLogger,
...
export function CsvFileInterceptor(
  ...
  class MixinInterceptor implements NestInterceptor {
    ...
    constructor(
      ...
      this.trackerLoggerCreator = loggerHelperService.makeCreator(CsvFileInterceptor.name);
    }
    ...
    fileFilter(req: Request, file: MulterFile, cb: FileFilterCallback) {
      if (file.mimetype !== 'text/csv') {
        return cb(new BadRequestException(`${fieldName} 欄位只接受 csv 檔案類型`));
      }
      cb(null, true);
    }
    ...
    checkIfFileExists(file: MulterFile | undefined) {
      if (!file) {
        throw new BadRequestException(`必須上傳一份 csv 檔案（欄位 ${fieldName} 為空）`);
      }
    }

    /**
     * 檢查檔案內容。如果檔案內容不符合規定則會刪除檔案並擲出例外。
     * @param file Multer 檔案物件。
     * @param logger Logger 物件。
     * @throws `InvalidCsvFormatException`
     */
    async validateFile(file: MulterFile | undefined, logger: TrackerLogger) {
      try {
        await validateCsvContent(file.path, logger);
      } catch (err) {
    ...
    /**
     * 執行自 Request 中將 csv 檔案抽出、轉換並檢查的工作
    ...
    async intercept(context: ExecutionContext, next: CallHandler): Promise<Observable<any>> {
      ...
      await this.validateFile(request.file, logger);
...
/**
 * 裝飾器。用於 Controller Method 上，擷取並檢測使用者是否上傳正確的 csv 檔案。
 * @param fieldName 檔案放在 `multipart/form-data` 中的欄位名稱。
 * @param deleteFileAfterwards 是否在一回 Request/Response 處理完成後將檔案刪除。預設為 `true`。
 */
export function UploadCsvFile(fieldName: string, deleteFileAfterwards: boolean = true) {
  return UseInterceptors(CsvFileInterceptor(fieldName, deleteFileAfterwards));
}
