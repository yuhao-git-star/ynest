  ...
  class MixinInterceptor implements NestInterceptor {
    ...
    fileFilter(req: Request, file: MulterFile, cb: FileFilterCallback) {
      if (file.mimetype !== 'image/svg+xml') {
        return cb(new BadRequestException(`${fieldName} 欄位只接受 svg 檔案類型`));
      }
      cb(null, true);
    }
    ...
    checkIfFileExists(file: MulterFile | undefined) {
      if (!file) {
        throw new BadRequestException(`必須上傳一份 svg 檔案（欄位 ${fieldName} 為空）`);
      }
    }
    ...
    /**
     * 執行自 Request 中將 svg 檔案抽出、轉換並檢查的工作
...
/**
 * 裝飾器。用於 Controller Method 上，擷取並檢測使用者是否上傳正確的 svg 檔案。
 * @param fieldName 檔案放在 `multipart/form-data` 中的欄位名稱。
 * @param deleteFileAfterwards 是否在一回 Request/Response 處理完成後將檔案刪除。預設為 `true`。
 */
export function UploadSvgFile(fieldName: string, deleteFileAfterwards: boolean = true) {
  return UseInterceptors(SvgFileInterceptor(fieldName, deleteFileAfterwards));
}
